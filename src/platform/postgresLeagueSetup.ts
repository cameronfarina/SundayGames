...
`.trim(),
  ...
  async joinLeagueSeasonTeam(input: JoinLeagueSeasonTeamRepositoryInput): Promise<PlatformLeagueMembership | null> {
    const now = input.now ?? new Date();

    try {
      return await this.#client.transaction(async transactionClient => {
        if (input.invitationTokenHash !== undefined) {
          const invitationResult = await transactionClient.query<{ id: string }>(`
SELECT id
...
`.trim(), [input.invitationTokenHash, input.leagueId, input.seasonId, now]);
          if (firstRow(invitationResult) === undefined) {
...
`.trim(), [
          membershipIdFor(input.leagueId, input.userId),
          input.leagueId,
          input.userId,
          input.role,
          now,
