...
describe("platform shell UI", () => {
  ...
  it("keeps the full player board useful before a user creates or joins a league", () => {
    ...
    expect(platformShellHtml).toContain('id="league-create-manual"');
    ...
    expect(platformShellHtml).toContain('data-field="rowConfirmed"');
    expect(platformShellHtml).toContain('team.confidence !== "high" || (team.issues || []).length > 0');
    ...
    expect(platformShellHtml).toContain('showLeagueCreationReview(manualLeagueCreationReview(outcome))');
    ...
    expect(platformShellHtml.split(screenshotDisclosure)).toHaveLength(3);
