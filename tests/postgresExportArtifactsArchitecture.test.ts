...
import * as exports from "../src/platform/postgresExportArtifacts.js";

const platformRoot = join(process.cwd(), "src/platform");
const moduleRoot = join(platformRoot, "postgresExportArtifacts");

const productionFiles = async (): Promise<string[]> => {
  const entries = await readdir(moduleRoot, { withFileTypes: true }).catch(() => []);
  return [
    join(platformRoot, "postgresExportArtifacts.ts"),
    ...entries
...
describe("Postgres export artifact repository architecture", () => {
  it("preserves its public runtime surface", () => {
    expect(Object.keys(exports)).toEqual(["PostgresExportArtifactRepository"]);
  });
