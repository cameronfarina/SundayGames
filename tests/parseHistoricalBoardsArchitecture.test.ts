...
import * as historicalBoards from "../src/data/parseHistoricalBoards.js";

const dataRoot = join(process.cwd(), "src/data");
const moduleRoot = join(dataRoot, "parseHistoricalBoards");

const productionFiles = async (): Promise<string[]> => {
  const entries = await readdir(moduleRoot, { withFileTypes: true }).catch(() => []);
  return [
    join(dataRoot, "parseHistoricalBoards.ts"),
    ...entries
