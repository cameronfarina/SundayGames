...
describe("platform runtime config parsing", () => {
  it("reads web, Postgres, and worker settings from environment variables", () => {
    const config = readPlatformRuntimeConfig({
      ...
      MOCKD_PLATFORM_ADMIN_ACCOUNT_IDS: "account-1, account-2",
      MOCKD_PLATFORM_DRAFT_OPERATIONS_TIMEZONE: "Europe/Rome",
      MOCKD_PLATFORM_DRAFT_DIGEST_TRIGGER_TOKEN: "digest-trigger-token-at-least-32-characters",
    ...
    expect(config).toEqual({
      ...
      platformDraftOperations: {
        administratorAccountIds: new Set(["account-1", "account-2"]),
        timezone: "Europe/Rome",
        digestTriggerToken: "digest-trigger-token-at-least-32-characters",
