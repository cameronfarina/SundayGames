...
const directory = path.resolve("src/modeling/auctionEngine/nominationContext");
...
describe("auction nomination context architecture", () => {
  it("keeps nomination responsibilities focused and type safe", () => {
    expect(existsSync(directory)).toBe(true);
    if (!existsSync(directory)) return;

    const facade = path.resolve("src/modeling/auctionEngine/nominationContext.ts");
    const files = [
      facade,
      ...readdirSync(directory)
        .filter(name => name.endsWith(".ts"))
        .map(name => path.join(directory, name)),
      path.resolve("tests/auctionNominationContext.test.ts"),
      path.resolve("tests/auctionNominationContextArchitecture.test.ts"),
    ];
