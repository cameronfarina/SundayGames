...
2. Apply migrations before the web rollout:
   ...
   Migration `platform-account-onboarding-v27` follows v26. It creates `account_onboarding_profiles` in the authentication database and backfills every existing account as complete, so the mandatory signup wizard applies only to accounts created after the migration. The previous web process ignores the additive table. Application rollback does not require a database restore.
...
| Migrations | `DATABASE_URL="$PRODUCTION_DATABASE_URL" npm run platform:migrate` completed before web rollout, repeat-run output is safe, readiness reports no missing migrations through `platform-account-onboarding-v27` in v24→v25→v26→v27 order, and `league_season_draft_setups` plus `account_onboarding_profiles` exist. |
...
| Signup setup | A new account cannot dismiss the two required setup questions, a submitted step survives refresh, completion stays complete, and an account created before v27 remains exempt after the backfill. |
