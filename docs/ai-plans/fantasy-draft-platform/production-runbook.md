...
2. Apply migrations before the web rollout:
   ...
   Migration `platform-account-onboarding-v27` follows v26. It creates `account_onboarding_profiles` in the authentication database and initially backfills every existing account as complete. Migration `platform-account-onboarding-rollout-v28` follows v27 and removes that automatic exemption only for accounts created during the preceding five days that have no saved onboarding answers and have never created a league. Run v28 only where authentication and normalized league records share one Postgres database; automatic startup skips it for auth-only or split-client configurations. Migration `platform-account-onboarding-intent-both-v29` then reserves the additive `intent_both` boolean. This compatibility release keeps the combined option hidden and clears the flag on every legacy intent write. Fully drain pre-v29 processes before a later release exposes the combined option; v29 is then the safe rollback target. Application rollback does not restore exemptions removed by v28. Before an affected account saves answers, restore its completed profile to reverse the rollout; after answers exist, preserve them and roll forward.
