...
2. Apply migrations before the web rollout:
   ...
   Migration `platform-account-onboarding-v27` follows v26. It creates `account_onboarding_profiles` in the authentication database and initially backfills every existing account as complete. Migration `platform-account-onboarding-rollout-v28` follows v27 and removes that automatic exemption only for accounts created during the preceding five days that have no saved onboarding answers and have never created a league. Run v28 only where authentication and normalized league records share one Postgres database; automatic startup skips it for auth-only or split-client configurations. Migration `platform-account-onboarding-intent-both-v29` then reserves the additive `intent_both` boolean and must be fully deployed before the combined option is exposed. The combined release stores `Both` as legacy `live_draft` plus the flag and sends a phase-1-compatible request. A response from a draining phase-1 server does not advance the wizard, so the user can retry after the swap. Rollback to phase 1 reads the base intent and clears the flag on a later intent write. Application rollback does not restore exemptions removed by v28. Before an affected account saves answers, restore its completed profile to reverse the rollout; after answers exist, preserve them and roll forward.
...
| Migrations | `DATABASE_URL="$PRODUCTION_DATABASE_URL" npm run platform:migrate` completed before web rollout, repeat-run output is safe, readiness reports no missing migrations through `platform-account-onboarding-intent-both-v29` in v24→v25→v26→v27→v28→v29 order, authentication and normalized leagues share that database, and `league_season_draft_setups` plus `account_onboarding_profiles` exist. |
...
| Signup setup | A new account cannot dismiss the two required setup questions, a submitted step survives refresh, completion stays complete, and v28 reopens only unanswered accounts created during the preceding five days that have never created a league. Older accounts, answered profiles, and league creators remain exempt or complete. |
