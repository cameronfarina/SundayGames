...
The new platform work added durable API and repository foundations. This branch now also has a small browser shell for signup, login, current session, league loading, team claims, live-room actions, realtime updates, and final CSV artifacts. The broader board-centered product shell is still incomplete.
...
| --- | --- | --- | --- |
| 1. Accounts, sessions, league memberships | Browser foundation partially landed | Account/session service, Postgres account/session repository, API-level create/login paths, session-token hashing, tests, small signup/login/current-session/logout shell | Production cookie/session polish, invitation/seed workflow, richer user-facing account errors |
...
| 5. Simulation execution | Browser worker landed | Shared seeded engine in a dedicated browser worker, authoritative server-prepared inputs, private persisted history | Richer failure/retry UX and import/pricing/export job handlers where still unsupported |
...
| 8. User prep dashboard and workspace UX | Thin shell partially landed | Login gate, seeded league load, team claim controls, and route links exist in `platformShellUi` | Unified navigation, reusable board/team panels, private prep workspace, contextual expert/news panels |
| 9. Live draft room realtime | Browser workflow partially landed | Live draft domain, command parsing, validation, read-model/SSE contracts, draft export domain, small live-room create/start/sale/undo/end/export UI | Postgres live room event/projection repositories, unified live-mode UI using same board/team panels, room setup/share UX, draft-night rehearsal |
...
The backend now has several correct seams, and the platform shell can drive the first live-room path. Next work should make the app feel like one product:
